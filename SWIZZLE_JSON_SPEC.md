...
5. [アセット様式（アートディレクション）](#アセット様式アートディレクション)
6. [アセット (assets)](#アセット-assets)
...
> このセクションは `src/ai/v2/art-direction.json`（様式の単一定義源）の転記です。
> AssetGenerator（生成プロンプト）と ImageQualityChecker（画像QA採点）も同じ定義を参照しています。
> 画像・SVGアセットを作る場合は、ここで定義した様式に**必ず**従ってください。

**様式名: GALLERY（ミニマル・エディトリアル）**
無彩色を基調に、**ヴァーミリオン1色だけ**をアクセントに効かせる。装飾を足さず、余白とシンプルなシルエットで「作品集／美術館」のような佇まいを作る。派手・ネオン・かわいさ過多・グラデーション多用は様式違反。

### 描法（全アセット共通）

- flat vector（フラットなベクター）
- clean thin or no outline（細い、または無し。太い縁取りはしない）
- no drop shadow（影は付けない）
- minimal shading（陰影は最小限）
- generous negative space（余白をたっぷり取る）
- 形状言語: confident simple silhouettes / clean flat geometric shapes（自信のあるシンプルなシルエット、整理された平面的な幾何形状。ごちゃつかせない）

### カラーパレット（HEX）

| 役割 | HEX | 用途 |
|------|-----|------|
| primary / accent | `#FF3B1F` | ヴァーミリオン。画面で効かせる唯一のアクセント色（重要な1要素だけに使う） |
| secondary（ink） | `#111111` | インク。シルエット・主要面・罫線 |
| 紙（light） | `#FAFAF7` | 温かみのあるオフホワイト背景 |
| 罫線（line） | `#E6E6E0` | 区切り・薄い罫 |
| dark版 紙 | `#0E0E0E` | ダーク基調を使う場合の背景 |

**テーマ別の派生**（基調＝無彩色は共通、アクセント1色のみ差し替え可）:

| テーマ | アクセント |
|--------|-----------|
| default / 和風 | `#FF3B1F` ヴァーミリオン |
| ポップ | `#FF715B` コーラル |
| ファンタジー | `#7A5CFF` バイオレット |
| レトロ | `#E0793A` バーントオレンジ |
| サイバー | `#2EC4B6` ティール（ダーク基調） |
| ナチュラル | `#4C9A6B` セージグリーン |

### オブジェクト画像（白背景・単一・切り抜き前提）

- 純白背景 `#FFFFFF`（テクスチャ・グラデーション・背景シーンなし）
- 単一オブジェクトのみ、中央配置で枠の70〜80%
- インク `#111111` 主体の太くシンプルなシルエット + アクセント色を**1要素だけ**に使用
- 影なし、線は細いか無し、余白を広く
- アイコン的で凛とした、ギャラリー品質

### 背景画像（9:16・中央オープン）

- 紙 `#FAFAF7` もしくはインクの色面。グラデーションは使わない
- 薄いベースライングリッド or 細い罫線1本まで（任意）。広大な余白
- **中央は完全に空ける**（ゲームオブジェクトが上に重なる）
- アクセント色の小さな図形を隅に1つまで。文字・キャラクターは入れない
- 静かでプレミアムな「美術館の壁」のムード

### 禁止事項

NOT busy / NOT neon / NOT cute-overloaded / NOT gradient-heavy。
レインボー（多色）パレット、写実テクスチャ、ドロップシャドウ、文字・ラベル、複数要素のシーンは禁止。

### SVGで作る場合の指針

- `fill` はパレットのHEXを直接使う（ランダムHSLや原色多用はしない）
- シルエットは `secondary`（`#111111`）、アクセントは1要素だけ `primary`（`#FF3B1F` 等）
- `<filter>` のドロップシャドウや派手な `<linearGradient>` は使わない（影なし・色面主体）
- 背景SVGは紙色の `<rect>` + 細い罫線1本程度に留め、中央を空ける
