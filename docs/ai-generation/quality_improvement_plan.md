...
```
1. GameIdeaGenerator  → GameIdea（アイデア）
2. ImageGenerator     → 画像アセット
3. SoundGenerator     → 音声アセット
4. LogicGenerator     → GameScript（ロジック）
5. ComplianceChecker  → 仕様適合チェック（アドバイザリーのみ）
6. FunEvaluator       → 面白さ評価 ← 【現在の合否判定はここのみ】
```
