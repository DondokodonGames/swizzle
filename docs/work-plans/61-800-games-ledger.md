...
| mechanic / family | 確定メカニクス(40ID)と族(A〜H) | 変更は overrides 経由 |
| theme / theme_jp / theme_source | 割当テーマ。`existing`=既存の世界観コメント尊重(notes参照)、`assigned`=決定的ラウンドロビン割当 | 人間(変更可) |
| style_pack | ARCADE_ART_DIRECTION の時代パック。テーマ親和リストを巡回割当 | 人間(変更可) |
| bgm_direction | BGM方針(melody固有化が第一候補/代替プリセット) | — |
| duration_current / duration_target | 現MAX_TIMEと目標値(PLAY_GRAMMAR_V3 §4の帯域にクランプ)。`目視(…)`=MAX_TIME検出不能、バッチが判断 | — |
...
1. **テーマ**: 世界観コメントあり91本は尊重(キーワード一致すればプールのテーマ、なければ `custom`)。
   残りは族ごとのカウンタで24テーマプールを巡回。**同一slugの重複ゲームは隣接順に割当**するため
   グループ内でテーマが必ず異なる(gravity-flip×12 → 12通りの見た目に分岐)
2. **スタイルパック**: テーマの親和パックリスト(スクリプト内 `THEMES`)をテーマごとに巡回。
   70s MONO は意図的に最少(峻厳なパックのため)。分布はsummaryで監視
3. **バッチ**: 族ごとに(優先度→slug→id)順で10本刻み。同族で束ねるのはセッション内の
   パターン再利用のため。バッチ内テーマは巡回割当により全員別
4. **Wave**: バッチをP1本数(降順)→平均スコア(昇順)でソートし、10バッチ=1 Wave。
