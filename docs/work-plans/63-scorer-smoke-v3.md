...
## 検証記録（2026-08-18 / Wave 0 パイロット9本）

ゲート自体が機能しているか（合格を配っていないか）を確認した。

| 対象 | 静的(validator v3 + scorer) | 実行時スモーク |
|---|---|---|
| done 9本 | 全て valid、スコア 88〜95 で**台帳の score_after と完全一致** | 9/9 PASS（エラー0・警告0・GAME_END到達） |
| Wave 0 残り4本（todo） | 全て invalid、スコア 49〜54（NO_MECHANIC / NO_THEME / HOW_TO_PLAY_PRESENT） | — |

done と todo でスコアが 88〜95 / 49〜54 にはっきり分かれ、ゲートが素通しになっていないことを確認。

**検証で判明したこと**:
- `game-ledger.csv` が Wave 0 の3コミット分**再生成されておらず**、9本の計測値が
  リファクタ前のまま（例: 017 は score 64 のまま、melody/sprite 使用は 0 のまま）だった。
  再生成し、WP56 のコミット規約に「バッチの最後に games:ledger」を追記
- summary の「既知事実との突合」が、リファクタで**当然変わる**指標に ❌ を出していた。
  「初回計測(2026-07)との差分」表示に変更し、動いてはいけない指標
  （distinct slug / scanlines）との読み分けを注記
- インク被覆率で `190-wave-surf` の下段が 0。スクリーンショットを見ると欠落ではなく
  ディテールのない一枚塗りの地面。不具合ではないが遠景の作り込み余地
