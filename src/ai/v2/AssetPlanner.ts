...
背景は**「雰囲気の壁紙」ではなく、「ゲームを視覚的に説明するシーン」**として設計してください。

### 背景に必ず含めるゲームコンテキスト要素
プレイヤーが背景を見ただけで「このゲームで何をするか」が伝わること。

| ゲームタイプ | 背景に描くべきもの |
|------------|----------------|
| タップ・選択系 | 選択候補が置かれる空間、テーマ設定の舞台（クイズ番組のスタジオ、試験会場など） |
| キャッチ・落下物系 | 落下物が落ちてくる上部の発生源 + キャッチエリア（バスケット・桶・ネット）が下部に見える |
| ドラッグ・移動系 | ゴールエリアが視覚的に強調された場所（光るゴールライン、特定の場所） |
| 回避・移動系 | 移動レーン、障害物が来る方向が分かる環境 |
| 時間制限・連打系 | カウントダウン的な雰囲気、競技場や対戦の舞台 |

### 背景の description フィールドに含める内容
- **シーンの設定**: どんな場所か（例: 祭りの屋台、宇宙船の格納庫）
- **プレイゾーン**: 操作が発生する領域の視覚的表現（例: 下部に水の入ったたらい）
- **文脈オブジェクト**: ゲームのテーマを伝える環境要素（例: 屋台の提灯、木製の縁）
- **色と雰囲気**: テーマに合った色調（例: 縁日の温かい橙色の光）

❌ 避けるべき背景: 「シンプルな青いグラデーション」「抽象的な模様」「無地の背景」
