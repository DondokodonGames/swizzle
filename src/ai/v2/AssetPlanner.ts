...
背景は**「テーマの雰囲気を色と質感で表現するシンプルなアート」**として設計してください。
ゲームの内容やメカニクスは背景に含めません（それはゲームオブジェクトの役割）。

### 背景設計の原則

**シンプルさが最優先**: ゲームオブジェクトが前景に配置されるため、背景が複雑だとオブジェクトが見えにくくなります。

| 良い背景 | 悪い背景 |
|--------|---------|
| テーマカラーのグラデーション | ゲームの目標が描かれた場面 |
| 抽象的なパターン（波、星、雲など） | バスケット・ゴールポスト・ターゲット等 |
| テーマの雰囲気を伝える風景（遠景のみ） | 詳細なシーンや複雑なイラスト |
| シンプルな幾何学模様 | 矢印や方向性のキュー |

### 背景の description フィールドに含める内容
- **テーマの雰囲気**: どんな色調や質感か（例: 深海をイメージした濃い青のグラデーション、珊瑚のシルエット）
- **スタイル**: どんな描き方か（例: フラットイラスト、水彩風、抽象的なパターン）
- **色の方向性**: 明暗・色相（例: 暖色系、寒色系、ネオンカラーなど）
- 中央は必ず空けること（ゲームオブジェクト配置エリア）

✅ 目指す背景: 「シンプルで統一感のある背景」「オブジェクトが映える空間」
❌ 避けるべき背景: ゲームメカニクスを説明するもの、複雑なイラスト、詳細なシーン
...

{
  ...
  "background": {
    "type": "image",
    "description": "背景の説明。必ずゲームコンテキストを伝える視覚要素を含める（例: 「金魚すくいの屋台。下部に水の入った丸い桶のシルエット。周囲に縁日の提灯と木製の柵。温かいオレンジ色の光」「宇宙の戦場。上部に敵UFOが飛来してくる宇宙空間。下部にシールドジェネレーターの台座」）",
    "mood": "雰囲気",
    "scrollable": false
  },
  "audio": {
