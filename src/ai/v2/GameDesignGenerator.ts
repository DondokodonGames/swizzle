...
## 使える操作（コンセプトに合わせて自由に組み合わせる）
- **タップ**: オブジェクトをタップ → 消える/変化/成功/失敗
- **ドラッグ（followDrag）**: オブジェクトを指で動かす。ゾーンとの衝突で判定
- **スワイプ**: 方向（上下左右）を検出して判定
- **長押し**: 押している時間でカウンター増加・ゲージ蓄積・判定
- **衝突判定（collision）**: 動くオブジェクト同士が触れた時に反応
- **タイマー**: 時間経過で状態変化・成功・失敗
- **カウンター**: N個の**別オブジェクト**を1回ずつ操作して成功 ← ⚠️「同じオブジェクトをN回」は実装不可
- **フラグ管理**: 複数ステップの操作順序を管理（例: 1つ目タップ→フラグ→2つ目タップ→成功）

コンセプトの playerOperation をそのまま活かして設計してください。
「タップだから全部同じ」にならないよう、**何をどの順で/タイミングで/どの条件でタップするか**を具体的に設計してください。

## サバイバル・回避ゲームの必須パターン
（ドラッグ回避 + 障害物衝突 + 生存タイマーの組み合わせが必要）
1. プレイヤーをドラッグで移動（followDrag）
2. 障害物がプレイヤーと衝突 → 失敗 **← ★必須！これがないとAUTO_SUCCESS無限ループになる**
3. N秒生存 → 成功（time → success）
敵・障害物には必ず自動移動（movement: wander または straight）を設定すること。
...

{
  "coreExperience": {
    ...
    "keyMechanic": "このゲームの核心操作を自由に記述（例：「仲間外れの魚をタップ」「ゴミをゴミ箱にドラッグ」「正しい方向にスワイプ」「ちょうど3秒で長押しを離す」など）"
  },
  "coreLoop": {
...
export class GameDesignGenerator {
  ...
  private extractAndParseJSON(text: string): GameDesign {
    return robustParseJSON<GameDesign>(text);
  }
