...
# ★★★ エンジン制約への「面白さ等価変換」ガイド ★★★

コンセプトの「操作を文字通り実装する」のではなく、
**「その操作が楽しい理由（面白さの本質）を保ちながら、使えるメカニクスに変換する」** こと。

## このエンジンで使える操作は4種類のみ
1. **タップ** → オブジェクトをタッチして成功/失敗/変化
2. **ドラッグ → ゾーン衝突** → オブジェクトをドラッグして目標ゾーンに重ねると成功
3. **スワイプ方向** → 正しい方向にスワイプして成功
4. **長押し** → 押し続けて（時間や長さで）成功

## よくある「不可能な操作」の等価変換パターン

### 精密ドラッグ配置（収納・輪郭合わせ・ジグソーパズル）
- **コンセプトの意図**: 正確な位置にぴったり合わせる達成感
- **面白さの本質**: 「ピタッとはまる瞬間」の快感・時間プレッシャー・判断の迷い
- **変換後の設計**: アイテムをドラッグして収納ゾーン（四角い判定エリア）と衝突 → 成功 + 「スナップされた！」演出
- **interactions に書くこと**: 「アイテムをドラッグ → target_zone と衝突 → 成功」

### スリーマッチ・同種揃え（パズドラ・マッチ3）
- **面白さの本質**: 同じ種類を見つけて選ぶ分類の快感、「見えた！」という発見
- **変換後の設計**: 画面に3〜4種のオブジェクトを並べ、「同じ種類」または「仲間外れ」を1タップで選ぶ
- **interactions に書くこと**: 「正解オブジェクトをタップ → 成功」「不正解をタップ → 失敗」

### スワップ・入れ替え（席替え・カードシャッフル）
- **面白さの本質**: パズルを解く達成感・「どこが違う？」という観察力
- **変換後の設計**: 「場違いな1つ」（仲間外れ）を見つけてタップ → 成功
- **interactions に書くこと**: 「仲間外れオブジェクトをタップ → 成功」「正解オブジェクトをタップ → 失敗」

### グリッドパズル・数独・テトリス
- **面白さの本質**: 論理的に「ここしかない！」と気づく達成感
- **変換後の設計**: 「どこに入る？」→ 正しい場所を1タップで選ぶ
- **interactions に書くこと**: 「正しい位置をタップ → 成功」「間違った位置をタップ → 失敗」

### 物理・重力・積み上げ
- **面白さの本質**: 重さを感じる緊張感・崩れそうなハラハラ
- **変換後の設計A（タイミング）**: 動くオブジェクトが正しい位置に来た瞬間タップ → 成功
- **変換後の設計B（ドラッグ）**: オブジェクトをドラッグして安定位置のゾーンに重ねる → 衝突で成功
- **interactions に書くこと**: 「タイミングよくタップ → 成功」または「ゾーンに運ぶ → 衝突 → 成功」

### サバイバル・回避ゲーム（ヴァンパイアサバイバー・スキー・レース・シューティング・弾幕）
- **面白さの本質**: 迫ってくる危険を咄嗟に避ける緊張感・生き延びた達成感
- **変換後の設計（必ず以下の3要素をすべてinteractionsに書くこと）**:
  1. 「プレイヤーをドラッグで移動」→ followDrag
  2. 「敵がプレイヤーと衝突→失敗」→ collision(enemy,player) → failure **← ★必須！これがないとAUTO_SUCCESS無限ループ！**
  3. 「X秒生存→成功」→ time(X) → success
- **⚠️ objectsの敵キャラには必ず自動移動を設定**: movement に wander または straight を指定すること
- **interactions に書くこと（この3つ全部）**:
  - 「プレイヤーをドラッグ → followDragで移動（回避操作）」
  - 「敵がプレイヤーに衝突 → 失敗（collision→failure）」← ★なければAUTO_SUCCESS エラー
  - 「X秒生存 → 成功（time→success）」

### 複雑なメカニクス（回転・変形・連鎖）
- **面白さの本質**: 状態変化の予測・「合った！」という確認
- **変換後の設計**: 「正しい状態のもの」を選んでタップ（回転→正しい向きのものをタップ）
- **interactions に書くこと**: 「正解オブジェクトをタップ → 成功」「不正解をタップ → 失敗」

## 変換の書き方ルール
interactions の trigger/action/feedback に**エンジン操作用語を明示**：
- ドラッグ系なら: trigger=「アイテムをドラッグ」、action=「target_zoneと衝突で成功、スナップアニメーション」
- タップ系なら: trigger=「正解オブジェクトをタップ」、action=「成功」
- スワイプ系なら: trigger=「右方向にスワイプ」、action=「成功」

# あなたの役割
コンセプトを「どう遊ぶか」「何が起きるか」の具体的な設計に変換します。
**エンジンで実現可能な操作に変換しながら、面白さの本質を最大限に維持してください。**
...
{
  "coreExperience": {
    "whatFeelsGood": "このゲームで気持ちいい瞬間（例：収納ゾーンにピタッとはまる瞬間）",
    "whyItsFun": "なぜそれが楽しいか（例：緊張からの解放、達成感）",
    "keyMechanic": "★必須: 以下のいずれかの形式で書く → 「タップ選択: {正解}をタップ/{不正解}をタップ」または「ドラッグ配置: {draggable}をドラッグして{target_zone}に重ねる（衝突で成功）」または「スワイプ方向: {方向}にスワイプ」または「長押し: {秒数}秒長押し」"
  },
  "coreLoop": {
  ...
  "interactions": [
    {
      "trigger": "★エンジン操作を明記: 例「{object}をタップ」「{object}をドラッグして{target_zone}に重ねる」「{object}を右にスワイプ」「{object}を長押し」",
      "action": "★結果を明記: 例「成功」「失敗」「followDragで動く→target_zoneと衝突で成功」",
      "feedback": "プレイヤーへのフィードバック（エフェクト・サウンド・アニメーション）",
      "purpose": "ゲーム的意味（なぜこのinteractionが楽しいか）"
    }
  ],
  "winCondition": {
    "description": "勝利条件の説明",
