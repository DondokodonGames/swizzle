...
# ★★★ 最重要: コンセプトとデザインを忠実に実装 ★★★

**コンセプトとデザインの内容をそのまま仕様に落とし込んでください。**
勝手にゲーム内容を変えないでください。

## チェックリスト（必ず確認）
- [ ] コンセプトの playerGoal が実現できるルールがあるか？
- [ ] コンセプトの successCondition を判定するルールがあるか？
- [ ] コンセプトの failureCondition を判定するルールがあるか？
- [ ] デザインの全オブジェクトが仕様に含まれているか？
- [ ] アセットプランにあるオブジェクトIDと一致しているか？

## よくある間違い ❌
- コンセプトで「金魚を3匹すくう」→ 仕様で「ターゲットを3つタップ」に変わってしまう
- テーマ固有のオブジェクト（金魚、星、猫など）を汎用的な「target」に置き換えてしまう
- 成功条件の数値（3匹、5個など）を勝手に変えてしまう

## 正しい仕様作成 ✅
- コンセプトの具体的なオブジェクトをそのまま仕様に記述
- 成功/失敗条件の数値をコンセプトから正確にコピー
- デザインで定義されたインタラクションをルールに変換
