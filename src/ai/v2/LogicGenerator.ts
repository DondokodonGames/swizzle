...
## 1. シンプルに作る（5-10秒のゲーム）
- オブジェクト数: 1〜5個で十分
- カウンター: **使う分だけ定義する**（成功/失敗条件で使わないカウンターは作らない）
- ルール数: 3〜8個程度
- 複雑なロジックは不要。1対1の対応で良い
  - 例: タッチ→非表示+カウンター増加、カウンターが目標値で成功

## 1.5 カウンターは必ず両方向で使う
カウンターを定義したら、必ず以下の両方を満たすこと:
- **操作するルール**: タッチやcollision時にincrement/add
- **チェックするルール**: 成功/失敗条件でカウンター値をチェック
**絶対に避ける**: 操作されないカウンター、チェックされないカウンター
...
- ゲーム開始直後に成功/失敗しない
- 成功条件がカウンターに依存する場合:
  - カウンターを増加させるルールが必要（タッチ→increment等）
  - 目標値に到達できる十分なオブジェクト数が必要
    - 例: 目標値5なら、タッチ可能なオブジェクトが5個以上必要
