...
- 複雑なロジックは不要。1対1の対応で良い
  - 例: 全部タッチしたら成功、間違ったものをタッチしたら失敗

## 1.5 カウンターは必要な時だけ使う
カウンターを使わなくても良いゲームは多い:
- ✅ 「全部のリンゴをタッチしたら成功」→ 各リンゴをhide、全部hiddenになったらsuccess
- ✅ 「爆弾を避けてリンゴをタッチ」→ リンゴタッチでsuccess、爆弾タッチでfailure
- ❌ 無意味にカウンターを作らない

カウンターを使う場合は:
- **操作するルール**: タッチやcollision時にincrement/add
- **チェックするルール**: 成功/失敗条件でカウンター値をチェック
両方が必要。片方だけのカウンターは作らない。
...
## 3. 成功条件は必ず到達可能にする
- ゲーム開始直後に成功/失敗しない
- 成功条件にはプレイヤー操作（touch/collision等）へのパスを必ず含める
- time条件のみで成功させない
- 成功までの道筋を確認する:
  - 例1: 「リンゴ3個タッチで成功」→ リンゴが3個以上存在するか？
  - 例2: 「全オブジェクトを非表示で成功」→ 全オブジェクトをhideできるルールがあるか？
