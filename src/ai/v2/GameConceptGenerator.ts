...
- **標準**: 5〜10秒（ほとんどのゲームはこの長さ）
- **やや長め**: 15秒（複数回の操作が必要な場合のみ）
- **最長**: 30秒（特別な理由がある場合のみ、滅多に使わない）

短いほど良いです。プレイヤーは「一瞬で終わる」ゲームを期待しています。
...
2. **操作明確性**
   ...
   - ✅ 具体的な操作（タップ/スワイプ/ドラッグ/長押し）が明示されている
   - ❌ 「うまくやる」

3. **判定明確性**
   成功と失敗の基準が数値で明確
   - ✅ 「◯個集める」「◯回成功」「◯点以上」など数値条件がある
   - ❌ 「なんとなく終わる」「時間切れで終了」
...
**同じようなゲームパターンを避けてください。**

## 使いすぎ禁止パターン ❌
以下のパターンは使いすぎです。**絶対に使わないでください**:
1. 「上から落ちてくるものを下で受ける」
2. 「複数のターゲットを順番にタップして消す」
3. 「光った順番を覚えて同じ順番で押す」（サイモン/記憶ゲーム）
4. 「ランダムに出現するものをタップする」（もぐら叩き）
5. 「左右に動いてアイテムを集める」

## 代わりに使うべきメカニクス ✅
以下のような**ユニークなメカニクス**を考えてください：

**位置判断**: 正しい場所を1回でタップ（間違いに見えるものと正解を見分ける）
**タイミング判断**: 動いているものが特定の位置に来た瞬間にタップ
**選択問題**: 複数の選択肢から正しいものを1つ選ぶ
**ドラッグ配置**: オブジェクトを正しい場所にドラッグ&ドロップ
**スワイプ操作**: 正しい方向にスワイプ
**長押し調整**: 長押しの長さで結果が変わる（力加減、チャージなど）
**傾き/重力**: オブジェクトを傾けて誘導する
**線を引く**: 2点を結ぶ線を引く
**回転**: オブジェクトを回転させて正しい向きにする
**フリック**: 正しい方向にフリックして飛ばす

## 良いゲームの例
- 「影と本体を一致させる」（影をドラッグして合わせる）
- 「正しい星座を選ぶ」（複数の星座から1つを選ぶ）
- 「バランスを取る」（傾きを調整してボールを落とさない）
- 「合言葉を入力」（正しい順序でシンボルをタップ）
- 「弓を引いて的に当てる」（長押しで力を調整してリリース）

# ⚠️ エディター制約（重要：これを守らないとゲームが成立しない）

このゲームエディターは**物理演算を持ちません**。以下の概念は実装不可能です。絶対に使わないでください：

❌ **使えない概念**
- 重力・落下（自然落下、バウンス、転がり）
- 積み上げ・バランス物理（積んだものが崩れる、傾いて倒れる）
- 弾性・反射・衝突後の跳ね返り
- 液体・流体シミュレーション
- 連鎖反応（ドミノ倒し、玉ころがし）

❌ 追加禁止: 自律移動するNPC・敵キャラ
- 「ゾンビが近づいてくる」「敵が追いかけてくる」「ボールが転がってくる」
  のような、NPCが自分でキャラクターに向かって移動するゲームは作れません。
- このエディターはNPCのAI移動を持ちません。
- 「敵が動く」系のゲームは → 「敵が固定配置されていて、プレイヤーが
  ドラッグで避ける」か「敵をタップして倒す」形に変換してください。

例: 「ゾンビが四方から迫ってくる」→ ❌ 実装不可
変換例: 「4方向に固定配置されたゾンビをタイミングよくタップして倒す」→ ✅

❌ 追加禁止: 「障害物・敵が一定方向に流れてくるのを避けるゲーム」
これはスクロール系ゲームの典型パターンですが、このエンジンでは正しく動作しません：
- 「コーンが上から流れてきて、車が避ける」
- 「隕石が降ってくるのを宇宙船でよける」
- 「壁が迫ってくるのをジャンプで乗り越える」
- 「魚がスクロールしてくる水族館レーン」

問題: コーンや隕石を「上から流してループさせる」複雑なルールをAIが正しく生成できない。
コーンが固定位置に置かれたまま動かず「車がコーンに絶対届かない」ゲームになります。

✅ 同じ「スラローム・回避」テーマを実現する代替設計:
- 「画面左/右にコーンが交互に出現する。正しい方向にスワイプして通過する」
  （コーンは動かない。タイミング判断を問う）
- 「3本のコースが表示される。コーン（障害物）がないコースをタップして選ぶ」
  （コーンは固定配置。判断力を問う）
- 「コーンをタップして弾く。3個全て弾けば成功」
  （コーンは固定。反射神経を問う）

❌ 追加禁止: コードで確認済みの未実装機能
- **オブジェクト間の物理反発**: 衝突しても押し戻しや弾き飛ばしは起きない
- **NPCの重力・物理シミュレーション**: 自然落下するオブジェクトが他オブジェクトに物理的にぶつかるゲームは不可
- **pause/restart アクション**: 動作しない

✅ **使えるメカニクスの実例**
- オブジェクトのタップ → 消える/変わる/成功
- ドラッグして正しい位置に置く（snap to target）
- スワイプ/フリックで方向を入力する
- 長押しでゲージが貯まり、離したら判定
- カウンターで「N個の別オブジェクトを1つずつタップして成功」
- 衝突判定（あらかじめ動いているオブジェクトが別のものに触れたら）
- タイマーで「N秒以内にタップ」
- ドラッグ回避（followDragで動かしてcollisionで判定）

**変換の考え方**: 物理に頼るコンセプトは「タップ/ドラッグで直接判定する」形に変換してください。
例: 「積み上げる」→「正しい位置にドラッグする」
例: 「落ちてくるものを積む」→「タイミングよくタップして選ぶ」
...
## ジャンル（テーマとゲーム内容に合わせて選択）
アクション/パズル/リズム/シューティング/収集/タイミング/記憶/反射神経/避けゲー/連打/仕分け/バランス

## タグ（テーマから連想される要素を3〜5個）
タグはテーマの世界観・雰囲気・文化的背景を反映すること。
例：
- テーマ「掛け軸の龍」→ タグ: 和風, 伝統, 龍, 墨絵, 迫力
- テーマ「宇宙ステーション」→ タグ: SF, 宇宙, 無重力, 未来, メカ
- テーマ「深海魚」→ タグ: 神秘的, 暗闘, 発光, 深海, 静寂

ランダムなタグではなく、テーマと一貫性のあるタグを選んでください。

# ⚠️⚠️⚠️ 最重要: playerOperationに操作方法を必ず明記 ⚠️⚠️⚠️

**playerOperationには以下の操作方法のいずれかを必ず含めてください:**
- タップ（tap）: 画面や対象を1回タッチ
- スワイプ（swipe）: 方向を指定してなぞる（左/右/上/下）
- ドラッグ（drag）: 対象を掴んで移動
- 長押し（hold）: 押し続ける

**❌ NG例（操作方法が不明確）:**
- "正しいものを選ぶ" → どうやって選ぶ？
- "バランスを保つ" → どう操作してバランスを取る？
- "敵を倒す" → タップ？スワイプ？
...

{
  ...
  "duration": 7,
  "theme": "テーマ（指定されたものを使用）",
  "visualStyle": "ビジュアルスタイル（テーマに合わせて選択）",
  "genre": "テーマとゲーム内容に合ったジャンル",
  "tags": ["テーマから連想されるタグ1", "タグ2", "タグ3"],
  "playerGoal": "プレイヤーの目標（具体的に、数値を含む）",
  "playerOperation": "★必須: 操作方法を明記（例: ○○をタップ / 左右にスワイプ / ドラッグで移動 / 長押しでチャージ）",
...
**duration（秒）の目安**:
- 5〜7秒: 1回の操作で終わるゲーム（タップして当てる、正しいものを選ぶ等）
- 8〜10秒: 2〜3回の操作が必要なゲーム（複数のターゲットをタップ等）
- 15秒: それ以上の操作が必要な場合のみ
- 30秒: 滅多に使わない（複雑なパズルのみ）
...
❌ 追加禁止: 「障害物が一定方向に流れてくるのを避けるゲーム」
- 「コーンが上から流れてきて車が避ける」→ ❌ コーンは動かず衝突不可のゲームになる
- 「隕石が降ってくるのを宇宙船でよける」→ ❌ 隕石が固定のまま
- 「壁が迫ってくるのをかわす」→ ❌ 壁が動かない

✅ 代替: 「3つのコースのうちコーンがないコースをタップして選ぶ」
✅ 代替: 「コーンをタップして弾く。3個全て弾けば成功」
...
**変換の考え方**: 物理に頼るコンセプトは「タップ/ドラッグで直接判定する」形に変換してください。
例: 「ジャンプして土管を超える」→「タイミングよくタップして障害物を避けたことにする」
例: 「積み上げる」→「正しい位置にドラッグする」
例: 「落ちてくるものを受ける」→「タイミングよくタップして選ぶ」
