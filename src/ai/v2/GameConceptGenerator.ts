...
❌ 追加禁止: コードで確認済みの未実装機能
以下はエンジンソースコードを調査した結果、実際には動作しないと確認された機能です：
- **パーティクルエフェクト**（particles）: 型定義はあるが描画されない
- **フェードイン/アウト**: show/hide の fade パラメータは無視される（即時切替のみ）
- **スワイプ/フリック条件**: swipe・flick のタッチ条件は現状発火しない
