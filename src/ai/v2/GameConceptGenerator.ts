...
**5〜10秒で終わる超短時間のマイクロゲームを設計してください。**
...
# 権利ルール（違反案は採用されません / docs/specifications/IP_SAFETY_RULES.md）
- ジャンルとメカニクスは自由に借りてよい。固有名詞・キャラクター・独特のUIは完全オリジナルにする
- 実在のタイトル名・キャラクター名・権利者名を title / description / tags に入れない
- 「〇〇風」という形で発想しない・名付けない。作るのは「このメカニクスの、完全オリジナルの見た目」
...
4. もぐら叩き専用メカニクス「ランダムに出現するものをタップするだけ」のゲーム（※random条件という**機能**自体はOK。それ**のみ**で成立する単調なゲームがNG）
5. 「左右に動いてアイテムを集める」
...
**傾き/重力（プレイヤー制御）**: プレイヤーの操作（スワイプ/タップ）で重力方向を変えてオブジェクトを誘導する ← setGravityで実装可能
**線を引く**: 2点を結ぶ線を引く
...
❌ **使えない概念**
- 自然落下（オブジェクトが自動で落下する重力・バウンス・転がり）
  ※プレイヤーが重力方向を操作するゲームはOK（上記「傾き/重力（プレイヤー制御）」参照）
...
/**
 * lean 版 CONCEPT_PROMPT（WP11: プロンプトダイエット）
 *
 * 4つの明確性（ゲームの定義）と出力契約は維持しつつ、使いすぎ禁止リスト・長大なテーマ例・
 * アーキタイプ強制を削る。テーマはコード側で themes.json から乱択して注入する（維持）。
 * 多様性は「類似度ゲート＋動的パターン分布の提示＋テーマ乱択」が担保するため、
 * プロンプトでは縛らず発想を広げる。
 */
const CONCEPT_PROMPT_LEAN = `あなたはスマホ向け超短時間ミニゲーム（マイクロゲーム、5〜10秒）のゲームデザイナーです。
強くなったあなたの発想力を信じています。奇抜でも、まだ誰も作っていないゲームを自由に考えてください。
...
# 絶対に守る前提条件（これはゲームの定義であり、発想を縛る制約ではない）
1. 目標明確性: 画面を見た瞬間に「何をすべきか」がわかる（具体的な対象と目標がある）
2. 操作明確性: タップ / スワイプ / ドラッグ / 長押し のどれで操作するか playerOperation に明記する
3. 判定明確性: 成功・失敗の基準が明確（数値があると良い。時間切れは補助条件）
4. 納得感: 結果がプレイヤーの操作で決まり、理不尽でない

# ハード制約（エンジンの現実。これだけは守る）
- 物理演算なし: 自然落下・反発・転がり・流体・ドミノ連鎖は不可（プレイヤーが重力方向を操作するのはOK）
- NPCのAI追尾なし: 敵が自分から向かってくるゲームは不可。固定配置 or 一定速度で流す（左右ドラッグで避ける等）に変換する
- 入力はシングルタッチのみ（タップ/スワイプ/ドラッグ/長押し）。同時マルチタッチ不可
- 画面はスマホ縦。制限時間は 5〜10秒（標準）、長くても 30秒

# 多様性について（縛らず、広げる）
似たゲームの量産を避けたいだけです。テーマ・操作・モチーフを自由に組み合わせ、新鮮なアイデアを優先してください。
（似すぎたゲームは後段の類似度ゲートが自動で弾くので、思い切って発想して構いません。）

# duration（秒）の目安
- 5〜7秒: 1回の操作で終わる / 8〜10秒: 2〜3回の操作 / 15秒以上: 複雑な場合のみ（稀）
...
# 出力（JSON形式。JSONのみ出力）
{
  ...
  "duration": 7,
  "theme": "テーマ（指定されたものを使用）",
  "visualStyle": "テーマに合うビジュアルスタイル",
  "genre": "テーマとゲーム内容に合ったジャンル",
  "tags": ["テーマから連想されるタグ1", "タグ2", "タグ3"],
  "playerGoal": "プレイヤーの目標（具体的に）",
  "playerOperation": "★必須: 操作方法を明記（例: ○○をタップ / 左右にスワイプ / ドラッグで移動 / 長押しでチャージ）",
  "successCondition": "成功条件（数値があると良い）",
  "failureCondition": "失敗条件（時間切れは補助条件として可）",
  "selfEvaluation": { "goalClarity": 8, "operationClarity": 8, "judgmentClarity": 8, "acceptance": 8, "reasoning": "各項目1文ずつ理由" }
}`;
...
export class GameConceptGenerator {
  ...
  async generate(feedback?: string): Promise<GameConcept> {
    ...
    let prompt = promptStyle === 'lean' ? CONCEPT_PROMPT_LEAN : CONCEPT_PROMPT;
...
成功条件・失敗条件の厳しさは想定難易度に合わせて設計してください。`;
  ...
  /**
   * メカニクスをインスピレーションとして提示するセクションを生成（lean 時のみ）
   *
