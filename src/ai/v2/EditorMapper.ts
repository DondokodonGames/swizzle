...
# ★★★ 最重要: 全オブジェクトへのルール完全生成 ★★★

## 絶対に守るルール

**インタラクティブなオブジェクトがN個ある場合、そのオブジェクト全てに対して完全なルールセットを生成すること。**
1個目のオブジェクトだけにルールを作って残りを省略することは絶対に禁止。

### ドラッグ操作ゲームの場合（drag系）
- draggable なオブジェクトが block_1, block_2, block_3 の3つなら:
  - drag_block_1, drag_block_2, drag_block_3 の3つのドラッグルールを生成する
  - place_block_1, place_block_2, place_block_3 の3つの配置判定ルールを生成する
  - 合計6ルール（省略禁止）

### タップ操作ゲームの場合（tap系）
- タップ対象が target_1, target_2, target_3 の3つなら:
  - tap_target_1, tap_target_2, tap_target_3 の3つのタップルールを生成する

### カウンター整合性ルール
- 成功条件のカウンター目標値（例: placed_count >= 5）は、
  実際にルールが存在してカウンターをインクリメントできる回数と一致させること。
  block_1のみにルールがある場合、目標値は1にすること（5は設定禁止）。

## オブジェクト配置の分離ルール
- 操作対象オブジェクト（移動させるもの）と、目標位置ガイド（配置先）は、
  必ず異なる座標に配置すること。
  - 操作対象: 画面下半分（y: 0.55〜0.85）
  - 目標ガイド: 画面上半分（y: 0.15〜0.45）
- 同じ座標に配置するとゲーム開始直後に衝突判定が発火して即終了する。
