...
## 1. 全オブジェクトへのルール完全生成（省略禁止）

**生成前に以下を確認すること（チェックリスト）：**
1. Specificationに登場する全オブジェクトIDをリストアップする
2. 各オブジェクトに必要なルール（ドラッグ/タップ/衝突）を列挙する
3. リストの全件に対してルールを生成したか確認してから出力する

「block_1を例として書いたので他も同様」という省略は禁止。
必ずblock_1, block_2, block_3...の全件を個別に明示すること。

インタラクティブなオブジェクトがN個ある場合、全N個に対してルールを生成すること。
1個目だけにルールを作って残りを省略することは絶対に禁止。

例: ゾンビが zombie_north, zombie_east, zombie_south, zombie_west の4体なら
  - タップ対象なら4つのタップルールを生成する
  - 衝突対象なら4つの衝突ルールを生成する

ドラッグ操作ゲーム（drag系）の場合:
- block_1, block_2, block_3 の3つなら drag×3 + place×3 = 合計6ルール（省略禁止）

カウンター整合性: 成功条件のカウンター目標値は実際にインクリメントできる
ルール数と一致させること（例: block_1のみにルールがあれば目標値は1）。

## 2. collision条件のAND/OR演算子ルール

複数のオブジェクトのどれかに触れたら発動させたい場合 → operator: "OR"
全てに同時に触れたときだけ発動させたい場合 → operator: "AND"

❌ NG（4体のどれかに触れたらダメージ、のはずがANDになっている）:
{
  "operator": "AND",
  "conditions": [
    {"type": "collision", "target": "zombie_north"},
    {"type": "collision", "target": "zombie_east"}
  ]
}
...
## 3. duration と success時間の整合ルール（★★★ 座標の制約セクションにも同じ内容あり）

settings.duration.seconds に設定した秒数がゲームの強制終了時間。
time条件で success を発火させるなら、その seconds は
settings.duration.seconds 以下でなければならない。

❌ NG: settings.duration.seconds=5 なのに time条件 seconds=7 で success（実例: コーンスラローム）
  （5秒でゲームが終わるため7秒のルールは永遠に発火しない）
✅ OK: settings.duration.seconds=7 で time条件 seconds=7 で success

## 4. 障害物スクロールパターン（★重要: 正しい実装方法）

「コーンが流れてくる」「隕石が降ってくる」など障害物が一定方向に動き続けるゲームは
以下の4ルールセットで実装する。これ以外の方法は動作しない。

### 必須ルール構成（例: コーンが上から下に流れる）

**ルール1: ゲーム開始時に障害物を動かし始める（各障害物ごとに1ルール）**
- 条件: time, timeType: 'exact', seconds: 0  ← ゲーム開始直後に1回だけ発火
- アクション: move, movement.type: 'straight', movement.direction: 'down', movement.speed: 3
- ※ speed は一度設定すれば毎フレーム自動で動き続ける（再発火不要）

**ルール2: 障害物が画面外に出たら上に戻す（各障害物ごとに1ルール）**
- 条件: collision, target: 'stageArea', collisionType: 'exit'
- アクション: move, movement.type: 'teleport', movement.target: {x: 0.3, y: 0.0}  ← 上端に戻す
- ※ x座標は各障害物ごとに違う値を設定して分散させること

**ルール3: プレイヤーキャラクターを左右ドラッグで操作**
- アクション: followDrag, constraint: 'horizontal'  ← 左右のみに制限

**ルール4: 障害物とプレイヤーの衝突で失敗**
- 条件: collision, target: <プレイヤーのobjectId>, collisionType: 'enter'
- アクション: failure

### 障害物の初期配置
- プレイヤー: 画面下部 y: 0.75〜0.85
- 障害物: 画面上部〜中部 y: 0.0〜0.5 に縦方向にずらして配置
  （複数コーンは y: 0.1, 0.3, 0.5 のように分散させる）
- 障害物の x 座標: 0.2〜0.8 の範囲で分散

### 絶対禁止
❌ 障害物を固定配置してプレイヤーが届かない位置に置く
❌ 移動ルール（ルール1）を生成し忘れる
❌ プレイヤーをステージ上部・障害物をステージ下部に配置する（上下が逆になる）

## 5. スワイプ移動のアクション

スワイプ操作でキャラクターを移動させる場合、followDragは使わない。
followDragはドラッグ（指を画面に押し当てたまま動かす）専用。
スワイプ（素早くはじく操作）には applyImpulse や move アクションを使うこと。

## 5. ブロック/ガイドの座標分離ルール

ドラッグして配置するゲームで移動元オブジェクトと配置先ガイドを使う場合:
- 移動元（ブロック等）: y 0.55〜0.85（画面下半分）に配置
- 配置先ガイド: y 0.15〜0.45（画面上半分）に配置
- 同じ座標に置くとゲーム開始直後に衝突判定が即発火する
...
## ★★★ オブジェクト配置ルール（3項目すべて必須）★★★

### 【1】同一座標禁止
複数のオブジェクトを同じ (x, y) 座標に配置しないこと。
最低でも x か y のどちらかが 0.1 以上異なる必要がある。

出力前に確認: 全オブジェクトのx,y座標をリストアップし、重複がないことを確認すること。

❌ NG例: cone_1=(0.25, 0.10), cone_3=(0.25, 0.10) → 完全一致
✅ OK例: cone_1=(0.25, 0.10), cone_3=(0.75, 0.10) → x が 0.5 異なる

### 【2】水平分散ルール
3個以上のオブジェクトが存在する場合、x座標を 0.2〜0.8 の範囲に分散させること。
x座標が左側（0.2〜0.4）だけに集中したり、右側だけに集中したりしないこと。

✅ 目標: 全オブジェクトのx座標の標準偏差が 0.15 以上になるよう配置する

### 【3/最重要】duration と success 時間の整合
settings.duration.seconds の値と、time条件で success を発火させる seconds を必ず一致させること。
settings.duration.seconds で指定した秒数が経過するとゲームが強制終了する。
それより大きい seconds を time条件に設定すると success は永遠に発火しない。

❌ NG: settings.duration.seconds=5 なのに success の time 条件 seconds=7
  （5秒でゲーム強制終了 → success が発火する前に終わる）
✅ OK: settings.duration.seconds=7 かつ success の time 条件 seconds=7
    ...
    "rules": [
      {
        ...
        "priority": 50,  // priorityDesign.rulePriorities から設定（省略可）
        "triggers": {
          "operator": "AND",
          "conditions": [ /* 条件配列 */ ]
        },
