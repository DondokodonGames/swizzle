...
## 3. duration と success時間の整合ルール

settings.duration.seconds に設定した秒数がゲームの強制終了時間。
time条件で success を発火させるなら、その seconds は
settings.duration.seconds 以下でなければならない。

❌ NG: settings.duration.seconds=5 なのに time条件 seconds=15 で success
  （5秒でゲームが終わるため15秒のルールは永遠に発火しない）
✅ OK: settings.duration.seconds=15 で time条件 seconds=15 で success
